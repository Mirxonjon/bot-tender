...
🟢 TO'G'RI KELADI (Мы можем реализовать проект, требования соответствуют нашим компетенциям, риски и затраты приемлемы)
🟡 QISMAN TO'G'RI KELADI (Проект в целом соответствует, но есть существенные ограничения или требуются субподрядчики/партнеры)
🔴 TO'G'RI KELMAYDI (Проект существенно не соответствует компетенциям, возможностям или экономической модели)
После статуса обязательно дай конкретное объяснение.

3. ЕСЛИ ЛОТ НЕ ПОДХОДИТ — ОБЯЗАТЕЛЬНО ОБЪЯСНИ ПОЧЕМУ
(Если статус «TO'G'RI KELMAYDI», укажи конкретные причины: Причина → Факт из ТЗ → Почему это проблема для нас → Насколько критично. Не используй пустые общие фразы).
